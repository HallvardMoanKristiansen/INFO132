...
public class MessageDatabase
{
    ...
    public static ArrayList<Message> getMessagesTo(Person recipient) {
        ...
        for(Message message : allMessages) {
            Person reciever = message.getRecipient();
    ...
    public static ArrayList<Message> getMessagesFromFriendsTo(Person recipient) {
        ArrayList<Message> allMessages = getMessages();
        ArrayList<Message> Hombres = new ArrayList<Message>();
        for (Message message : allMessages){
            if (recipient.getName().equals(message.getRecipient().getName())){
                for(Person recipientHombres : recipient.getFriends()){
                    if (recipientHombres.getName().equals(message.getSender().getName())){
                        Hombres.add(message);
                    }
                }
            }
